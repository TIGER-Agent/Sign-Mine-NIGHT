// app.js (v8.0 - Final WASM Initialization Fix)
const main = async () => {
    console.log('[App] DOM is ready. Initializing WASM library...');
    try {
        await cardano_serialization_lib.load();
    } catch (e) {
        console.error("Failed to load WASM module", e);
        alert("Lỗi nghiêm trọng: Không thể nạp các thành phần cốt lõi của Cardano.");
        return;
    }
    ...
    const updateUIState = (state) => {
        ...
        if (state === 'initial') {
            ...
            connectBtn.disabled = true; showStatus('Đang kết nối...', 'info');
        } else if (state === 'connected') {
            signBtn.disabled = false; disconnectBtn.disabled = false;
        } else if (state === 'signing') {
            signBtn.disabled = true; disconnectBtn.disabled = true; showStatus('Đang chờ người dùng ký...', 'info');
        }
    };
    const resetState = (reason = '') => {
        walletApi = null; stakeAddress = null; updateUIState('initial');
    ...
    const handleConnect = async () => {
        ...
        if (typeof window.cardano === 'undefined') { showStatus('Lỗi: Không tìm thấy extension ví Cardano.', 'error'); return; }
    ...
    const handleSign = async () => {
        ...
        if (!walletApi || !stakeAddress) { resetState('Lỗi trạng thái kết nối.'); return; }
        ...
        try {
            ...
            const resultData = { owner: stakeAddress, publicKey: signedData.key, signature: signedData.signature, paymentAddress: paymentAddress };
